...
`React Fiber` — это механизм рендеринга в React, который был представлен в версии 16.

### **`Как автор понимает принцип работы React Fiber:`** 

1. Впервые собирается дерево React элементов
...
Сигнатура аналогична `useEffect()`. В большинстве случаев рекомендуется использовать `useEffect()`.

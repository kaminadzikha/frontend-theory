...
В React ref (или ссылка) — это способ получить доступ к DOM-элементам или компонентам, созданным с помощью React,
напрямую, а не через состояние или пропсы. Это может быть полезно для управления фокусом, выполнения анимаций,
интеграции с внешними библиотеками и прочих задач, где требуется взаимодействие с DOM напрямую.
...
Это можно избежать, используя контекст API или состояние управления в более высокоуровневом компоненте.
...
Предохранители (Error Boundaries) — это механизм в React для обработки ошибок, которые могут возникать в компонентном
дереве. Они позволяют перехватывать ошибки JavaScript в компонентах, а также выводить запасной UI вместо того, чтобы
приложение ломалось и отображало стандартную ошибку.
...
1. **Использование `React.memo`**: Этот компонент позволяет избегать ненужных повторных рендеров путем мемоизации
   компонента. `React.memo` сравнивает предыдущие и следующие пропсы и перерисовывает компонент только при изменении
   пропсов.

2. **Оптимизация рендеринга списка**: Используйте ключи (`key`) для элементов в списке, чтобы React мог эффективно
   отслеживать элементы и избегать лишних операций.

3. **Избегание inline-функций и объектов**: Создание функций и объектов в рендер-методе может привести к повторным
   рендерам. Лучше использовать `useCallback` и `useMemo` для мемоизации функций и значений.

4. **Использование `useCallback` и `useMemo`**: Эти хуки помогают мемоизировать функции и вычисляемые значения, чтобы
   избежать их пересоздания при каждом рендере.

5. **Lazy loading и код сплиттинг**: Загрузка компонентов по требованию с использованием `React.lazy` и `Suspense` может
   значительно улучшить время загрузки приложения.

6. **Оптимизация рендеринга с помощью `shouldComponentUpdate`**: В классических компонентах вы можете переопределить
   этот метод для контроля, когда компонент должен перерисовываться.

7. **Virtualization**: Использование библиотек для виртуализации, таких как `react-window` или `react-virtualized`,
   помогает эффективно рендерить длинные списки.

8. **Избегание ненужных обновлений состояния**: Если состояние обновляется слишком часто, это может замедлить
   приложение. Оптимизируйте частоту обновлений и используйте функциональные обновления состояния, если это возможно.

9. **Оптимизация CSS**: Минимизируйте количество CSS правил и избегайте сложных селекторов, которые могут замедлить
   рендеринг.

10. **Обработка больших данных и сложных вычислений вне рендеринга**: Для сложных вычислений используйте Web Workers или
    перемещайте их в другие части приложения, чтобы не блокировать основной поток рендеринга.
...
## 60. **Назовите основную цель React Fiber?**
...
2. **Приоритизация**: Fiber поддерживает приоритизацию обновлений, позволяя выполнять более важные задачи в первую
